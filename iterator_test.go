...
func TestDB_Iterator_Multi_Values(t *testing.T) {
	...
	assert.Nil(t, err)
	err = db.Put([]byte("ssnue"), utils.RandomValue(10))
	assert.Nil(t, err)
	err = db.Put([]byte("dsadb"), utils.RandomValue(10))
	assert.Nil(t, err)
	err = db.Put([]byte("acvgg"), utils.RandomValue(10))
	assert.Nil(t, err)
